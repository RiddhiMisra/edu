...
const Catalog = () => {
    ...
    const { catalogName } = useParams();//python
    const [catalogData, setCatalogData] = useState([]);//python ke courses
    const [categoryId, setCategoryId] = useState(null);//python ka categoryid
    const [active, setActive] = useState(1)

    const getCategories = async () => {
        const result = await apiConnector("GET", categories.CATEGORIES_API);//fetches all categories,webd python etc
        const id = result?.data?.categories.filter((ct) => { return ct.name.toLowerCase() == catalogName.toLowerCase() })[0]._id;//matching category ka id nikal leta h,suppose python ki id
        setCategoryId(id);
    }
    const getCategoryDetails = async () => {
        const result = await getCatalogPageData(categoryId);//pyhton caetgory id se related sare courses 
